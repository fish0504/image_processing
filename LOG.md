...
### やったこと
* ハンドの逆運動学と順運動学のデバッグ用コードを書いて、目標座標から関節角がひじ上げの状態で求まるようにした
* 逆運動学をハンド制御プログラムに実装して、目標位置を予め設定してその位置に逆運動学で追従する動きをつくった<br>
  ->実際に静止したターゲットに対してつかめるか実験して確認した。うまく指をひっかけることができたときの動画が取れた。

### 次にやること
* 今回の実験でどの位置にコップをおけばハンドでつかめるかわかったので、コップの位置が決定できる。<br>次はコップとカメラ(Realsense)の位置関係を固定して、Dex-Netの出力(dx,dy,depth)をハンドの目標位置(x,y)へ変換する式を求める

* コップの位置が確定できるようになったので、姿勢推定の調整とうまくいくようにコップのマーキングを行う。
  ->時系列で正しく推定できているか、UDP通信しながらホストPCで確認できるようにする.
  
* dexNetをrealsenseの後に実行する時にセグフォが発生しているのでそれを直す(単体では実行できている)<-cpuのメモリとか? boost::pythonの問題?


## 0201

### やったこと
* 実験環境の設定とDex-Net出力からハンド目標位置へ変換する式の導出、パワポで図を作成
* デバッグ
* 姿勢推定のための角度に対応した画像を自動で取って、姿勢推定を再び行った。<br>
    ->背景が暗幕でないとあまりうまくいかないようなので、次は背景を暗幕でやってみる、場合によってはハンドを黒く覆う
    
### 次やること
* 姿勢推定を今の状態で再現する(Basler版とRealsense版)
* dex-netからの出力を固定した状態で逆運動学までうまくいくようにデバッグする
* なぜかdexNetがRealsenseのあとに実行したときに落ちるバグをデバッグする
